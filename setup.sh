...
stage_4_apptainer() {
    if ! command -v apptainer >/dev/null; then
        cat >&2 <<EOF
FAIL: apptainer not installed.

setup.sh expects apptainer to be either (a) pre-installed on the host,
or (b) staged into $BUNDLE_ROOT/apt/ as .deb files so stage 3 can
install them offline.

Fix on the build host:
  bash scripts/build-sif-bundle.sh             # collects apt/ on 24.04
  # — or, if your build host is not 24.04, get the .deb closure once
  # from a 24.04 container/VM (see build-sif-bundle.sh --help).

Then rsync the bundle + rerun setup.sh.
EOF
        exit 1
    fi
...
# Detect the GPU's compute capability via nvidia-smi (e.g. "10.0" for
# B200/B300, "12.0" for RTX 50, "9.0" for H100). Empty when SKIP_GPU or
# nvidia-smi unavailable. Echoed only; callers route the value into env.
_gpu_arch() {
    [[ $SKIP_GPU -eq 1 ]] && return 0
...
_write_vllm_env() {
    ...
    local ARCH; ARCH="$(_gpu_arch)"
    note "resolved $model_id -> quant=${M_QUANT:-none} tp=$M_TP max_len=$M_MAXLEN parser=$M_PARSER port=$port arch=${ARCH:-unknown}"
    {
        ...
        echo "VLLM_USE_V1=1"
        # Blackwell (sm_100 B200/B300, sm_120 RTX 50): pin the arch so
        # any residual FlashInfer / torch.compile path doesn't mis-detect
        # it. flashinfer-python is also stripped from the SIF at build
        # time (containers/vllm.def), so this is belt-and-braces.
        if [[ -n "$ARCH" ]]; then
            echo "TORCH_CUDA_ARCH_LIST=$ARCH"
            case "$ARCH" in
                10.0|10.*|12.0|12.*)
                    echo "VLLM_USE_FLASHINFER_SAMPLER=0"
                    ;;
            esac
        fi
    } > "$env_file"
