...
| `setup.sh` read `/etc/loopcoder/*.yaml` but nothing copied them — fresh B300 would fail at stage 7 / stage 9 | Stage 0 now seeds `install.yaml`, `vllm.yaml`, `loopcoder.yaml` from `bundle/source/LoopCoder/config/*.yaml.example` when missing; fails loudly if a template is missing too | ✅ Unit-verified |
